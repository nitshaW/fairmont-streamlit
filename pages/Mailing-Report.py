...
if mandrill_df is not None and conversion_df is not None:
    ...
    st.write(f"Records in Mandrill DF for selected date range: {len(mandrill_df)}")
    st.write(f"Records in Conversion DF for selected date range: {len(conversion_df)}")

    # Check if there's data after filtering
    if mandrill_df.empty and conversion_df.empty:
